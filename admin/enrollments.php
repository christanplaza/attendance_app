...
if (isset($_GET['id'])) {
    if ($conn) {
        ...
        $sql = "SELECT c.title, c.description, c.status, c.id AS class_id, u.first_name, u.last_name, e.grade, e.enrollment_end
            FROM classes c
            JOIN enrollments e ON c.id = e.class_id
            JOIN users u ON c.teacher_id = u.id
            WHERE e.student_id = " . $id . " AND e.status = 'active' AND e.enrollment_end >= CURDATE()";
